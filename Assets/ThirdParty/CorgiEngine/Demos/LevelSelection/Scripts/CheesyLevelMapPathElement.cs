...
{	
	...
	/// </summary>
	public class CheesyLevelMapPathElement : MonoBehaviour 
	{

		private RetroAdventureProgressManager progressManager;
		[Tooltip("level is approached from above; player can go back that way")]
		public bool AccessedFromAbove;
		[Tooltip("level is approached from right; player can go back that way")]
		public bool AccessedFromRight;
		[Tooltip("level is approached from left; player can go back that way")]
		public bool AccesedFromLeft;
		[Tooltip("level is approached from below; player can go back that way")]		
		public bool AccessedFromBelow;
		...
		public CheesyLevelMapPathElement Up;
		...
		public CheesyLevelMapPathElement Right;
		...
		public CheesyLevelMapPathElement Down;
		...
		public CheesyLevelMapPathElement Left;
		...
		private bool IsLevelComplete(){
			...
			if(progressManager != null){
				LevelSelector level = GetComponent<LevelSelector>();
				for(int i = 0; i < progressManager.Scenes.Length; i++){
					if(
					level != null 
					&& progressManager.Scenes[i].SceneName == level.LevelName 
					&& progressManager.Scenes[i].LevelComplete == false){
						return false;
					}
				}
			}
		...
		public virtual bool CanGoUp()
		{
			...
			if (Up!=null && (IsLevelComplete() || AccessedFromAbove)) { return true; } else { return false; }
		}
		/// <summary>
		...
		{
			if (Right!=null && (IsLevelComplete() || AccessedFromRight)) { return true; } else { return false; }
		}
		/// <summary>
		...
		{
			if (Down!=null && (IsLevelComplete() || AccessedFromBelow)) { return true; } else { return false; }
		}
		/// <summary>
		...
		{
			if (Left!=null && (IsLevelComplete() || AccesedFromLeft)) { return true; } else { return false; }
		}
		...
		protected virtual void Start () 
		{
			...
			progressManager = FindObjectOfType<RetroAdventureProgressManager>();
		}
		...
		public virtual void OnTriggerStay2D(Collider2D collider)
		{
			...
			CheesyLevelMapCharacter mapCharacter=collider.GetComponent<CheesyLevelMapCharacter>();
